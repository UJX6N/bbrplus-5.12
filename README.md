# bbrplus-5.12
Linux BBRplus Kernel 5.12 ported from BBRplus 4.14  
(note that it does NOT based on 5.12 version of BBR, instead just simple ported the 4.14 version of BBRplus)
<br/>
<br/>
<br/>
...
###  ii)  keep official v5.12 tcp_bbr module in the kernel, now can do either  
<br/>
...
### 1) get convert patch on this repository, use git or direct download
        (e.g., convert_official_linux-5.12.x_src_to_bbrplus.patch)
...
### 2) download officaial linux kernel
        say 5.12.19       
            wget https://cdn.kernel.org/pub/linux/kernel/v5.x/linux-5.12.19.tar.gz

<br/>
<br/>

### 3) extract the tarball & cd extracted directory
        tar zxvf linux-5.12.19.tar.gz && cd linux-5.12.19
...
### 4) copy convert patch to extracted kernel directory
        something like
            cp ../convert_official_linux-5.12.x_src_to_bbrplus.patch .

<br/>
<br/>

### 5) do the patch job
        patch -p1 < convert_official_linux-5.12.x_src_to_bbrplus.patch
...
### 8) disable debug info & module signing
        scripts/config --disable SECURITY_LOCKDOWN_LSM && scripts/config --disable DEBUG_INFO && scripts/config --disable MODULE_SIG
